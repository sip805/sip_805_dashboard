// ══════════════════════════════════════════════════════════════
// SignUp — Create a new winery owner account
// After sign-up, App.jsx checks Firestore and routes to ClaimWinery
// ══════════════════════════════════════════════════════════════

import { useState } from "react";
import { Wine, AlertCircle } from "lucide-react";
import { createAccount, signInWithGoogle } from "../firebaseClient.js";
...
export default function SignUp({ onSwitchToSignIn }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // onAuthChange in App.jsx will pick up the new user and route accordingly
    } catch (err) {
      const msg = err.code === "auth/email-already-in-use"
        ? "An account with this email already exists. Try signing in."
        : err.message?.replace("Firebase: ", "")?.replace(/Error \(.*\)/, "") || "Sign up failed.";
      setError(msg);
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-purple-200 text-sm mt-1">Sign up to claim your winery dashboard</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
          ...
          <p className="text-center text-sm text-gray-500 mt-5">
            ...
            <button onClick={onSwitchToSignIn} className="text-purple-600 font-semibold hover:underline">Sign In</button>
          </p>
        </div>
      </div>
    </div>
